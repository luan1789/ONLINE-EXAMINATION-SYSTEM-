...
public class UpdateController extends HttpServlet {
    ...
    private static final String ERROR = "EditQuestionController";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            for (int i = 1; i <= 4; i++) {
                int answerID = Integer.parseInt(request.getParameter("txtAnswerID" + i));
                ...
                listAnswer.add(new AnswerDTO(answerID, answer_content, isCorrect));
            }
            String sub = request.getParameter("Sub");
            boolean status = "Active".equals(request.getParameter("Status"));

            QuestionDTO ques = new QuestionDTO(questionID, question_content, listAnswer, new java.util.Date(), sub, status);
            QuestionDAO dao = new QuestionDAO();
            if (dao.update(ques) != -1) {
                url = SUCCESS;
            }
